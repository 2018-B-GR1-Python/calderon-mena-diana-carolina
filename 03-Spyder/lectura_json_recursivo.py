...
directorio_a_leer = "C:\\Users\\Carolina\\Downloads\\pandas-fundamentals\\02\\demos\\demos\\collection-master\\artworks"

ejemplo_json = directorio_a_leer + "\\a\\000\\a00001-1035.json"

llaves_a_usarse = ['id', 'all_artists', 'title', 'medium', 'classification', 'height','width','units']
...
def obtener_registros_de_archivos_json (path_archivo, llaves):
    ...
    arreglo_valores = []
    for llave in llaves:
        arreglo_valores.append(contenido_json[llave])
    return tuple(arreglo_valores)
    
registro_json = obtener_registros_de_archivos_json(ejemplo_json, llaves_a_usarse)
registo_json_dataframes = pd.DataFrame([registro_json])


directorio_json = directorio_a_leer
